...
const todayStr = new Date().toISOString().split('T')[0]; // Using simple today for DB query, doesn't matter much for KST afternoon

const emps = db.prepare(`SELECT id, name FROM employees WHERE name IN ('김강훈', '정세빈', '이동찬')`).all();
